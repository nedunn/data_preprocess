...
def grab_truncate(file, i_before=None, i_after=None):
    ...
    x0,y0=u.df.col_extract(df,0), u.df.col_extract(df, 1)
...
def frame_comparison(df, x_col='raman shift', y_col='intensity', file_col='spectra_name', frame_loc='-Frame-', save_dir=None):
    if save_dir==None:
        print('\nFrame Comparison figures will NOT be saved.')
        save=False
        pass
    else:
        print('\nFrame comparison figures will be saved.')
        save=save_dir
        u.check_path(save_dir)
    df=df.copy(deep=True)
    #Determine Frame Groups
    groupList=[]
    for name in df[file_col]:
        group=name.split(frame_loc)[0]
    ...
    for g in groups:
        ...
        if save==False:
            ...
            figure.save.web(fig,save_dir,g)
                
def frameAverage(df, rsCol, filename_col, frame_loc='-Frame-'):
...
if __name__=='__main__':
    ...
    for file in filelist:    
        name, df, fig = grab_truncate(file)
        df['spectra_name']=name
        DF=DF.append(df)
    #All data in 1 Dataframe
    DF.reset_index(inplace=True,drop=True)
    frame_comparison(DF,frame_loc='-Frame-') #,save_dir=indir)
    
    
    
    #DFave=frameAverage(DF, 'raman shift', 'spectra_name', frame_loc='-frame')
